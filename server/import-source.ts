...
export async function prepareImportSource(
  ...
  const professionPath = professionAbsolutePath(
    repositoryRoot,
    request.profession,
  );
  ...
  const relativePath = `${professionRelativePath(request.profession)}/.dnf-import-source-${request.runId}.md`;
  const absolutePath = resolveInside(repositoryRoot, relativePath);
  try {
    await assertNoSymlinkChain(repositoryRoot, absolutePath);

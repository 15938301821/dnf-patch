...
const atomicReplaceRetryDelaysMs = [10, 25, 50, 100, 200, 400, 800] as const;
const transientFilesystemErrorCodes = new Set(["EACCES", "EBUSY", "EPERM"]);
...
export async function writeFileAtomic(
  ...
  try {
    await writeFile(temporary, data, { flag: "wx" });
    await retryTransientFilesystemOperation(() => rename(temporary, path));
  } finally {
    await rm(temporary, { force: true });
  }
}
...
/**
 * Windows 索引器或实时防护可能短暂锁住刚读取的目标文件。只重试明确的共享
 * 冲突错误；路径、权限配置或数据错误保持立即失败，避免掩盖真实故障。
 */
export async function retryTransientFilesystemOperation(
  operation: () => Promise<void>,
  retryDelaysMs: readonly number[] = atomicReplaceRetryDelaysMs,
): Promise<void> {
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      if (!isTransientFilesystemError(error) || delayMs === undefined) {
        throw error;
      }
...
function isTransientFilesystemError(
  error: unknown,
  ...
  return (
    ...
    transientFilesystemErrorCodes.has(error.code)
  );
}

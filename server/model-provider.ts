...
export class AgentModelProvider {
  ...
  readonly #structuredStoragePolicy: ModelCallRecord["responseStoragePolicy"];
  ...
  constructor(request: Pick<RunRequest, "provider" | "allowNetwork">) {
    ...
    let endpoint: ReturnType<typeof resolveOpenAIEndpoint> | undefined;
    let configurationError: string | undefined;
    try {
      endpoint = resolveOpenAIEndpoint(process.env);
    } catch (error) {
      configurationError = safeError(error);
    }
    this.endpointIdentity = endpoint?.identity ?? "configuration-invalid";
    this.#configurationError = configurationError;
    this.#structuredStoragePolicy = endpoint?.custom
  ...
  /** 为每次调用补齐固定端点和超时证据，避免各返回分支遗漏审计字段。 */
  #record(
    input: Omit<
      ModelCallRecord,
      | "schemaVersion"
  ...
  /** 返回脱敏后的客户端不可用原因，不把 API Key 写入事件或证据。 */
  #clientUnavailableError(): string {
    if (!this.#apiKey) {
      return `${OPENAI_API_KEY_ENV} is not available in the process environment.`;
    }
    return (
      this.#configurationError ??
      "The OpenAI-compatible client could not be initialized."
    );
  }
    ...
    if (!this.allowNetwork) {
      return {
        record: this.#record({
          ...
          requestSha256,
          networkAuthorized: false,
          responseStoragePolicy: this.#structuredStoragePolicy,
          error:
    ...
    if (!this.#client || !this.#apiKey) {
      return {
        record: this.#record({
          ...
          responseStoragePolicy: this.#structuredStoragePolicy,
          error: this.#clientUnavailableError(),
        }),
      };
    }

...
- 产物生成或验证中发现的稳定视觉规律，只有在当前线程已通过结构、像素或联系表证据后，才可补充到职业或主题 Prompt。
- 职业 Prompt 只吸收跨主题成立的动作、轮廓、阶段、锚点、人物/特效边界和验收条件。
- 主题 Prompt 只吸收该主题下稳定成立的色板、材质、粒子、光线、构图增量和主题排除。
- 资源身份、NPK/IMG 路径、帧索引、输出哈希、构建目录、工具绝对路径、模型端点和单次运行参数不得写入 Prompt；这些只属于 manifest、run plan、验证报告或 release 证据。
- 回写后的 Prompt 必须仍满足本契约固定章节，并通过 `tools/Test-DnfPromptTree.ps1`；未通过时不保留该回写。

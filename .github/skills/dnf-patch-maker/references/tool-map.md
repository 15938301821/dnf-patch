...
当验证出的正确工程需要反馈到规则、Prompt 或 skill 时，按变更范围选择门禁：

- 只改职业/主题 Prompt 或 AGENTS：运行 `tools/Test-DnfPromptTree.ps1`，并检查本次回写没有新增未核验 NPK/IMG、绝对路径、覆盖声明或部署步骤。
- 改项目级 skill、根规则或 Prompt 契约：运行 `tools/Test-DnfProjectGate.ps1`，并复核 `.github/skills` 与 `.codex/skills` 的同名文件没有语义分叉。
- 改 PowerShell 脚本或 workflow：先运行 `tools/Test-DnfPowerShellSource.ps1`，再按 workflow 或发布契约运行对应 fixture 与项目总门禁。
- 资源身份、帧索引、哈希和产物路径不通过 Prompt 回写；它们由 manifest、validation 或 release 门禁复核。

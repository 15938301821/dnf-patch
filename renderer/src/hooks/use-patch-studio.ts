...
import { errorMessage, newRunId, splitSkills } from "../utils/run-format.js";
...
export function usePatchStudio(): PatchStudioController {
  ...
  useEffect(() => {
    const dispose = desktopApi.onRunEvent((event) => {
  ...
  const chooseDesignFile = useCallback(async (): Promise<void> => {
    try {
      const path = await desktopApi.selectDesignFile();
      if (path) {
        setForm((current) => ({
          ...current,
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const response = await desktopApi.startRun(request);

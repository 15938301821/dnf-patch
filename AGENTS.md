...
- 所有手工 P 图和栅格精修使用本机 Adobe Photoshop CC 2018。
- 尽量保留原帧作底层参考，使用非破坏图层、蒙版和可复核的导出设置。
- 导出后重新检查像素尺寸、alpha、边缘、色彩模式和文件名；Photoshop 预览不得代替全帧结构验证。
- AI 出图只作为素材或概念。回灌前必须在 Photoshop CC 2018 中按原帧几何、时序和 alpha 重新适配。
...
- AI 原始输出、Photoshop 适配稿和最终运行帧必须分目录保存；任何生成服务不得直接写入 NPK、覆盖源帧或把输出部署到 `ImagePacks2`。具体契约见项目级 `$dnf-patch-maker` 的 `references/frame-redraw-and-adapter-contract.md`。
...
5. 在 Photoshop CC 2018 中按源帧适配并导出运行帧；回灌接口的期望位图尺寸、alpha、色彩模式、图集和 mipmap 策略由实际 IMG handler 与 manifest 决定，不统一关闭 mipmap 或强制固定方形尺寸。
...
- 逐帧重绘任务的源帧、生成素材、Photoshop 适配稿和回灌帧引用闭合；每个实际变化帧都能追溯到唯一运行记录，缺帧、重帧、错序、尺寸漂移和未声明的生成配置漂移均为硬失败。

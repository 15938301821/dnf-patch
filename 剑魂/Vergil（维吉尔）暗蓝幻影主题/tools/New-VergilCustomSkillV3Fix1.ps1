...
$components = @(
    [pscustomobject]@{
        id        = 'illusionslash'
        config    = 'configs/custom-skill-v3-fix1/illusionslash.json'
        component = 'npk/custom-skill-v3-fix1/components/a_weaponmaster-vergil-dark-blue-illusionslash-v3-fix1.NPK'
        summary   = 'validation/custom-skill-v3-fix1/components/illusionslash/build-summary.json'
        include   = @(
    ...
    [pscustomobject]@{
        ...
        config    = 'configs/custom-skill-v3-fix1/illusionslash_finish.json'
        component = 'npk/custom-skill-v3-fix1/components/a_weaponmaster-vergil-dark-blue-illusionslash_finish-v3-fix1.NPK'
        summary   = 'validation/custom-skill-v3-fix1/components/illusionslash_finish/build-summary.json'
        include   = @(
    ...
    [pscustomobject]@{
        ...
        config    = 'configs/custom-skill-v3-fix1/illusionslash_finish-sparkhead04-stable.json'
        component = 'npk/custom-skill-v3-fix1/components/a_weaponmaster-vergil-dark-blue-illusionslash_finish-sparkhead04-stable-v3-fix1.NPK'
        summary   = 'validation/custom-skill-v3-fix1/components/illusionslash_finish-sparkhead04-stable/build-summary.json'
        include   = @(
...
$finalOutput = Resolve-ThemePath -RelativePath 'npk/custom-skill-v3-fix1/a_weaponmaster-vergil-dark-blue-custom-skill-v3-fix1.NPK'
$packageSummaryPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/package-summary.json'
$indexPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/independent-index.json'
$fullFrameDirectory = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/full-frame-validation'
$pixelRawPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/pixel-state-raw.json'
$pixelValidationPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/pixel-state-validation.json'
$validationSummaryPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3-fix1/validation-summary.json'
...
foreach ($component in $components) {
    ...
    foreach ($includePath in $component.include) {
        if ($includePath -notlike 'sprite/character/swordman/effect/illusionslash/*') {
            throw "Custom-skill-v3-fix1 may only include illusionslash IMG paths: $includePath"
        }
        if ($includePath -like '*cutin*' -or $includePath -like '*momentaryslash*') {
            throw "Custom-skill-v3-fix1 must not include Cut-in or momentaryslash paths: $includePath"
        }
        if (-not $includeSet.Add($includePath)) {
            throw "Duplicate custom-skill-v3-fix1 include path: $includePath"
        }
...
$summary = [ordered]@{
    ...
    status         = 'passed-offline-custom-skill-v3-fix1-client-pending'
    scope          = [ordered]@{
        profession                     = 'swordman-weaponmaster'
        themeId                        = 'weaponmaster-vergil-dark-blue'
        label                          = 'custom-skill-v3-fix1'
        ...
        note                           = 'Partial skill candidate: v3 plus the missed illusionslashvp2body.img layer. The a_ filename is only a client loading order test candidate; offline validation does not prove client priority.'
    }
    artifact       = [ordered]@{
    ...
    changes        = [ordered]@{
        ...
        addedAfterV3             = @('sprite/character/swordman/effect/illusionslash/illusionslashvp2body.img')
        excludedCutin            = $true
    ...
    evidence       = @(
        ...
        Get-FileSnapshot -Path (Resolve-ThemePath -RelativePath 'tools/New-VergilCustomSkillV3Fix1.ps1')
    )
    deployment     = [ordered]@{
        performed = $false

...
活动图像编辑契约已迁移到项目本地 Aseprite。当前 v5 只读门禁未报告 Aseprite 注册或 API 能力 blocker，但 31 个组件现场复核、帧 3–26 的 24 个分层工程、24 个 runtime PNG、新 Cut-in NPK、活动聚合包、final summary 和发布闭环仍未完成，因此活动状态为门禁阻断，`fullSkillCoverageProven=false`。
...
使用同一个 `RunId` 渲染并构建 Cut-in；两个入口均拒绝覆盖已有路径：
    ...
    & '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\tools\Render-CutinWeaponmasterNeoVergil.ps1' -RunId $runId
    & '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\tools\Build-VergilCutinWeaponmasterNeo.ps1' -RunId $runId
...
全量模型+Aseprite 生成的 Prompt 输入必须同时包含 `jobs\剑魂\prompts` 职业动作骨架、本主题 `AGENTS.md` 共同 Base Style/色板/材质/边界，以及本主题 `prompts` 逐技能视觉增量；主题 Prompt 单独不足以证明技能构图、阶段、轮廓和锚点正确。

当前活动资源计划为 `configs\full-skill-v1\resource-plan-v5.json`。它从不可变 v3 基线继承 31 个历史组件证据，使用精确的仓库相对 `剑魂/` 到 `jobs/剑魂/` 只读重定位策略，停用历史 Cut-in v2 复用项，并在组件与新 Cut-in 证据齐全前保持阻断。旧 v4 原字节保留且不再作为活动入口。活动计划门禁：

    & '.\tools\Test-VergilAsepriteMigrationPlan.ps1' -ResourcePlanPath '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\configs\full-skill-v1\resource-plan-v5.json'

只有计划返回 `ReadyForAggregation=true` 后，才可在新的版本化路径聚合 31 个组件与新 Cut-in，并使用新的活动最终验证入口写入新的空目录。不得覆盖历史 v1 package、final 或 release 证据。

## 正式自动化工作流

活动链已注册为 `weaponmaster.vergil.aseprite-full-skill-v2`，声明文件位于 `workflows\aseprite-full-skill-v2.json`。旧 v1 原字节保留且不再作为活动入口。v2 按固定顺序执行 PowerShell 源码门禁、本地工具链、迁移 readiness、32 源聚合、最终验证、人工审核模板、人工审核门禁、原子发布元数据、发布闭环和项目总门禁。

默认入口只做静态验证，不创建 Run 目录，也不运行 Aseprite、构建或发布：

    & '.\tools\Invoke-DnfWorkflow.ps1' -WorkflowPath '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\workflows\aseprite-full-skill-v2.json'

真实执行必须提供新的 `RunId` 和显式执行开关：

    $runId = 'weaponmaster-vergil-aseprite-20260714-v1'
    & '.\tools\Invoke-DnfWorkflow.ps1' -WorkflowPath '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\workflows\aseprite-full-skill-v2.json' -RunId $runId -Execute
...
完成审核后使用同一 `RunId` 恢复：
    ...
    & '.\tools\Invoke-DnfWorkflow.ps1' -WorkflowPath '.\jobs\剑魂\Vergil（维吉尔）暗蓝幻影主题\workflows\aseprite-full-skill-v2.json' -RunId $runId -Execute -Resume
...
当前 v3 基线保持迁移证据要求的长度 `383297` 和 SHA-256 `C9E65A869F21497E293976A113193550E297CCCC4D1853103E502A2232315EB8`。v5 现场验证仍受组件现场复核、24 帧活动工程/runtime、新 Cut-in 构建与独立验证、最终聚合、最终验证和发布闭环等 8 个真实 blocker 约束，因此仍不得执行聚合或宣称当前契约已发布。

...
$components = @(
    [pscustomobject]@{
        id        = 'illusionslash'
        config    = 'configs/custom-skill-v3/illusionslash.json'
        component = 'npk/custom-skill-v3/components/weaponmaster-vergil-dark-blue-illusionslash-v3.NPK'
        summary   = 'validation/custom-skill-v3/components/illusionslash/build-summary.json'
        include   = @(
    ...
    [pscustomobject]@{
        ...
        config    = 'configs/custom-skill-v3/illusionslash_finish.json'
        component = 'npk/custom-skill-v3/components/weaponmaster-vergil-dark-blue-illusionslash_finish-v3.NPK'
        summary   = 'validation/custom-skill-v3/components/illusionslash_finish/build-summary.json'
        include   = @(
    ...
    [pscustomobject]@{
        ...
        config    = 'configs/custom-skill-v3/illusionslash_finish-sparkhead04-stable.json'
        component = 'npk/custom-skill-v3/components/weaponmaster-vergil-dark-blue-illusionslash_finish-sparkhead04-stable-v3.NPK'
        summary   = 'validation/custom-skill-v3/components/illusionslash_finish-sparkhead04-stable/build-summary.json'
        include   = @(
...
$finalOutput = Resolve-ThemePath -RelativePath 'npk/custom-skill-v3/weaponmaster-vergil-dark-blue-custom-skill-v3.NPK'
$packageSummaryPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/package-summary.json'
$indexPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/independent-index.json'
$fullFrameDirectory = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/full-frame-validation'
$pixelRawPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/pixel-state-raw.json'
$pixelValidationPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/pixel-state-validation.json'
$validationSummaryPath = Resolve-ThemePath -RelativePath 'validation/custom-skill-v3/validation-summary.json'
...
foreach ($component in $components) {
    ...
    foreach ($includePath in $component.include) {
        if ($includePath -notlike 'sprite/character/swordman/effect/illusionslash/*') {
            throw "Custom-skill-v3 may only include illusionslash IMG paths: $includePath"
        }
        if ($includePath -like '*cutin*' -or $includePath -like '*momentaryslash*') {
            throw "Custom-skill-v3 must not include Cut-in or momentaryslash paths: $includePath"
        }
        if (-not $includeSet.Add($includePath)) {
            throw "Duplicate custom-skill-v3 include path: $includePath"
        }
...
$summary = [ordered]@{
    ...
    status         = 'passed-offline-custom-skill-v3-client-pending'
    scope          = [ordered]@{
        profession                     = 'swordman-weaponmaster'
        themeId                        = 'weaponmaster-vergil-dark-blue'
        label                          = 'custom-skill-v3'
        ...
        note                           = 'Partial skill candidate: only verified illusionslash technical IMG paths from the three custom-skill-v3 component configs are packaged. No Cut-in or momentaryslash IMG paths are included.'
    }
    artifact       = [ordered]@{

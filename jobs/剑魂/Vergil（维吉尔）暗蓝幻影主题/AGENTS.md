...
- 合法 Aseprite 或 API 能力证据缺失、24 帧工程/runtime/摘要未生成或新的独立最终验证未通过时，活动状态保持阻断，不得沿用旧 v1 已归档的覆盖结论宣称新契约已经完成。

## 正式 DAG 路由

- 当前主题正式 workflow ID 为 `weaponmaster.vergil.aseprite-full-skill-v2`，声明文件为 `workflows/aseprite-full-skill-v2.json`，活动计划为 `configs/full-skill-v1/resource-plan-v5.json`；职业 manifest 必须保持该注册关系，不能从 README 或历史 handoff 猜入口。旧 workflow v1 与 resource plan v4 只保留为迁移前历史字节证据，不作为活动入口。
...
- 当前 v5 只读门禁未报告 Aseprite 注册或 API 能力 blocker，但组件现场复核、24 帧分层工程/runtime、新 Cut-in 构建、独立验证、32 源聚合、最终验证和发布闭环仍未完成，因此只允许静态验证；不得执行正式写链、生成审核通过状态或提升活动覆盖结论。
